document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // PERBAIKAN 1: Path harus relatif dari N3/Lesson/ (naik 2 level)
        ...
        if (['kotoba', 'k', 'u'].includes(lesson.tipe)) {
            thead.innerHTML = '<tr><th>Kanji</th><th>Hiragana</th><th>Arti</th></tr>';
            lesson.data.forEach(item => {
                tbody.innerHTML += `
                    <tr>
                        <td>${item.Kanji || item.kanji || ''}</td>
                        <td>${item.Hiragana || item.hiragana || ''}</td>
                        <td>${item.Arti || item.arti || ''}</td>
                    </tr>`;
            });
        } 
            ...
            lesson.data.forEach(item => {
                const jidoushiText = `${item.jidoushi_kanji || ''} (${item.jidoushi_hiragana || ''}): ${item['jidoushi_arti'] || item['jidoushi_arti '] || ''}`;
                const tadoushiText = `${item.tadoushi_kanji || ''} (${item.tadoushi_bentuk_formal || item.tadoushi_hiragana || ''}): ${item['tadoushi_arti'] || item['tadoushi_arti '] || ''}`;
            ...
            lesson.data.forEach(item => {
                tbody.innerHTML += `<tr><td>${item.kotoba || ''}</td><td>${item.arti || item['arti '] || ''}</td></tr>`;
            });
        }
        else if (['gkk', 'kk'].includes(lesson.tipe)) {
        ...
        quizTypes.forEach(q => {
            ...
            // PERBAIKAN 2: Path harus naik 1 level ke N3, lalu masuk ke Quiz
            ...
            // Matikan tombol jika tidak relevan
        ...
        lessonTitle.textContent = "Error";
        lessonContent.textContent = "Gagal memuat data pelajaran.";
    }
});